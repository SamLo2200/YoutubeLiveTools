...
export default async function getVideoInfo(youtube_video_id: string) {
    ...
    // Error testing
    // const response = await fetch(`https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2C%20liveStreamingDetails&id=${youtube_video_id}&key=${process.env.API_KEY}s`);
